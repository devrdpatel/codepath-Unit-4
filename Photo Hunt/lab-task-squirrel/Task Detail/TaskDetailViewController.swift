    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        ...
        if PHPhotoLibrary.authorizationStatus(for: .readWrite) != .authorized {
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { [weak self] status in
                switch status {
                case .authorized:
                    // Present the image picker since the user allowed authorization
                    DispatchQueue.main.async {
                        self?.presentImagePicker()
                    }
                default:
                    // Show user settings alert to enable permission
                    DispatchQueue.main.async {
...
extension TaskDetailViewController: UIImagePickerControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        guard let result = info[.phAsset] as? PHAsset else {
            print("Could not load as asset")
            DispatchQueue.main.async { [weak self] in self?.showAlert(for: "Could not load as assest" as? Error) }
            return
        }
        
        guard let location = result.location else {
            print("No location found")
            DispatchQueue.main.async { [weak self] in self?.showAlert(for: "No Location found" as? Error) }
            return
        }
        
        guard let image = info[.editedImage] as? UIImage else {
            print("No Image found")
            DispatchQueue.main.async { [weak self] in self?.showAlert(for: "No Image found" as? Error) }
            return
        }
